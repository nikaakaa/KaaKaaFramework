...
public class InputBindingMgr : BaseManager<InputBindingMgr>
{
    ...
    private IInputActionCollection2 inputActionsWrapper; // 保存 wrapper 类实例
    ...
    /// 初始化InputActionAsset
    /// </summary>
    /// <param name="asset">InputActionAsset 实例</param>
    /// <param name="autoEnable">是否自动启用所有 ActionMap（默认 false，由使用者手动控制）</param>
    public void Init(InputActionAsset asset, bool autoEnable = false)
    {
        if (asset == null)
        {
        ...
        // 不自动启用所有 ActionMap，避免 Unity 内部机制尝试通过反射调用方法
        // 由使用者根据需要手动启用特定的 ActionMap
        if (autoEnable)
        {
    ...
    /// <summary>
    /// 初始化生成的 InputActions wrapper 类
    /// 这样改键时使用 asset，使用时可以用 wrapper 类
    /// </summary>
    /// <typeparam name="T">生成的 InputActions 类型（如 InputSystem）</typeparam>
    /// <param name="actions">生成的 InputActions 实例</param>
    /// <param name="autoEnable">是否自动启用所有 ActionMap（默认 false，由使用者手动控制）</param>
    public void Init<T>(T actions, bool autoEnable = false) where T : class, IInputActionCollection2
    {
        if (actions == null)
        {
            Debug.LogError("InputActions 不能为空！");
            return;
        }
        
        // 保存 wrapper 实例
        inputActionsWrapper = actions;
        
        // 通过反射获取 asset 属性（生成的类都有 asset 属性）
        var assetProperty = typeof(T).GetProperty("asset");
        if (assetProperty != null)
        {
            var asset = assetProperty.GetValue(actions) as InputActionAsset;
            if (asset != null)
            {
                Init(asset, autoEnable); // 使用 wrapper 的 asset，不自动启用
            }
            else
            {
                Debug.LogError("无法从 InputActions 获取 asset！");
            }
        }
        else
        {
            Debug.LogError($"类型 {typeof(T).Name} 没有 asset 属性！");
        }
    }
    
    /// <summary>
    /// 获取生成的 InputActions wrapper 类实例
    /// 用于类型安全的访问（如 InputSystem.Player.Move）
    /// </summary>
    /// <typeparam name="T">生成的 InputActions 类型</typeparam>
    /// <returns>InputActions 实例，如果类型不匹配或未初始化返回 null</returns>
    public T GetInputActions<T>() where T : class, IInputActionCollection2
    {
        return inputActionsWrapper as T;
    }
